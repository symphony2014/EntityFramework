...
{
    public class PropertyMappingConvention : IModelConvention
    {
        public virtual InternalModelBuilder Apply(InternalModelBuilder modelBuilder)
        {
            ...
            foreach (var entityType in modelBuilder.Metadata.EntityTypes)
            {
                ...
                if (entityType.HasClrType)
                {
                    ...
                    clrProperties.UnionWith(entityType.ClrType.GetRuntimeProperties().Where(pi => pi.IsCandidateProperty()).Select(pi => pi.Name));
                    ...
                    if (clrProperties.Count > 0)
                    {
                        ...
                        {
                            var actualProperty = entityType.ClrType.GetRuntimeProperties().First(pi => pi.Name == clrProperty);
